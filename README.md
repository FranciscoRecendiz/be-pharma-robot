...
🧬 BePharma AI Prospecting — v2.0
<p align="center"> <b>Descubrimiento y validación automatizada de empresas farmacéuticas y healthcare</b><br> <i>Una solución interna desarrollada por BePharma Analytics (Nov 2025)</i> </p> <p align="center"> <img src="https://img.shields.io/badge/Python-3.12-blue" /> <img src="https://img.shields.io/badge/Docker-Compose-orange" /> <img src="https://img.shields.io/badge/n8n-Orchestration-success" /> <img src="https://img.shields.io/badge/Groq-LLM-purple" /> <img src="https://img.shields.io/badge/Status-Stable-brightgreen" /> </p>
🚀 Propósito General
...
📊 Pandas / Asyncio / Tenacity	Procesamiento asíncrono y control de errores
🧩 Módulos Principales
find_companies_daily.py

Descubrimiento de empresas (Groq + DDG).

resolve_sites_post.py

Validación asincrónica de sitios oficiales.

site_resolver.py

Motor de resolución y limpieza de dominios.

icp_targets.csv

Input: combinaciones país × giro.

companies_seen.csv

Registro persistente de empresas vistas.

empresas_final.csv

Resultado final enriquecido y verificado.
...
services:
  n8n:
    ...
    environment:
      ...
      - LANGCHAIN_TRACING_V2=false
    volumes:
      - "./bp-analytics:/project"
      - "./bp-analytics/n8n_data:/home/node/.n8n"
    restart: unless-stopped
...
Nota: Define todas las credenciales y API Keys en un archivo .env (no se sube al repo).
...
1️⃣ Descubrimiento de empresas
Ejecuta el script de descubrimiento de empresas:
...
2️⃣ Resolución de sitios
Ejecuta el script de validación de sitios:

docker exec -it n8n-bepharma python3 -m src.pipelines.resolve_sites_post \
  --input /project/data/processed/venezuela/empresas.raw.csv \
  --output-dir /project/data/cleaned


Salida:

/data/cleaned/cleaned_empresas_raw.csv

/data/cleaned/log_resolver.json

3️⃣ Extracción de contactos (en desarrollo)
El script extract_contacts_from_sites.py buscará emails y cargos de decisión.
...
Configuración persistente de n8n en n8n_data/.

Los datos se deduplican automáticamente y se almacenan por país.

🧩 Guía de Ejecución

Ejecutar el proceso de descubrimiento (find_companies_daily) usando el siguiente comando para procesar los datos de venezuela:

docker exec -it n8n-bepharma python3 -m src.pipelines.find_companies_daily --country "Venezuela" --model "llama-3.3-70b-versatile" --daily-limit-combos 100 --batch-size 10 --ddg-max 20


Ejecutar la validación de sitios usando:

docker exec -it n8n-bepharma python3 -m src.pipelines.resolve_sites_post --input /project/data/proce